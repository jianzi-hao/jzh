    ...
    if info1[-3:] in ['jpg','png','jpeg','tif','bmp']:
        ...
        try:
            ...
            for i in results:
                ...
                if i[0] == 'smoking':
                    color = [0, 0, 255]
                    ui.printf(str(time.strftime('%Y.%m.%d %H:%M:%S ', time.localtime(time.time()))) + '警告！检测到吸烟人员')
                if i[0] == 'calling':
                    color = [0,0,255]
                    ui.printf(str(time.strftime('%Y.%m.%d %H:%M:%S ', time.localtime(time.time()))) + '警告！检测到手机')
...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.label.setStyleSheet("font-size:36px;font-weight:bold;font-family:SimHei;background:rgba(255,255,255,0.8);")
        ...
        self.pushButton.setStyleSheet("background:rgba(99,142,200,1);border-radius:10px;padding:2px 4px;")
        self.pushButton.setObjectName("pushButton")
        self.pushButton_2 = QtWidgets.QPushButton(self.centralwidget)
        self.pushButton_2.setGeometry(QtCore.QRect(1020, 810, 150, 40))
        self.pushButton_2.setStyleSheet("background:rgba(99,142,200,1);border-radius:10px;padding:2px 4px;")
        self.pushButton_2.setObjectName("pushButton_2")
        self.pushButton_3 = QtWidgets.QPushButton(self.centralwidget)
        self.pushButton_3.setGeometry(QtCore.QRect(1020, 870, 150, 40))
        self.pushButton_3.setStyleSheet("background:rgba(99,142,200,1);border-radius:10px;padding:2px 4px;")
    ...
    def retranslateUi(self, MainWindow):
        ...
        MainWindow.setWindowTitle(_translate("MainWindow", "司机抽烟或者打电话等违规行为检测系统"))
        self.label.setText(_translate("MainWindow", "司机抽烟或者打电话等违规行为检测系统"))
        self.label_2.setText(_translate("MainWindow", "请点击以添加文件"))
        self.pushButton.setText(_translate("MainWindow", "添加文件"))
        self.pushButton_2.setText(_translate("MainWindow", "文件检测"))
            ...
            try:
                ...
                for i in results:
                    ...
                    if i[0] == 'smoking':
                        color = [0, 0, 255]
                        ui.printf(
                            str(time.strftime('%Y.%m.%d %H:%M:%S ', time.localtime(time.time()))) + '警告！检测到吸烟人员')
                    if i[0] == 'calling':
                        color = [0, 0, 255]
                        ui.printf(
                            str(time.strftime('%Y.%m.%d %H:%M:%S ', time.localtime(time.time()))) + '警告！检测到手机')
                    cv2.rectangle(image, p1, p2, color, thickness=3, lineType=cv2.LINE_AA)
                    cv2.putText(image, str(i[0]) + ' ' + str(i[2])[:5], (int(box[0]), int(box[1]) - 10),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.75, color, 2)
            except:
